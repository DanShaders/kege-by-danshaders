...
std::vector<token_t> tokenize(std::string_view s, std::ostringstream &err) {
	...
	const auto OPS = std::initializer_list<std::pair<std::string_view, token_t::type_t>>{
	...
	for (std::size_t i = 0; i < s.size(); ++i) {
		...
		if (s[i] == '"' || s[i] == '\'') {
			...
			if (j == s.size()) {
				err << "Expected " << s[i] << " to close string started at " << i
					<< ", found EOF\n";
			}
...
struct expr_node {
	...
	expr_node(const token_t *token_) : token(token_) {}
...
const auto SHARED_ERROR_NODE = std::make_shared<error_node>(&SHARED_ERROR_TOKEN);
...
expr_tree build_tree(expr_build_context &ctx, std::size_t l, std::size_t r) {
	if (l == r) {
		if (l == 0) {
			...
			ctx.err << "Empty expression at indexes " << ctx.tokens[l - 1].l << " .. "
					<< ctx.tokens[r].l << "\n";
		}
	...
	auto merge_stacks = [&](int priority) {
		while (op_stack.size() && OP_PRIORITY[op_stack.back()->type] < priority) {
			...
			for (std::size_t i = op_stack.size() - j; i < op_stack.size(); ++i) {
				result =
					std::make_shared<binary_operator_node>(op_stack[i], result, expr_stack[i + 1]);
			}
...
bool filter::matches(const std::vector<int64_t> &vars) {
	...
	while (1) {
		...
		if (op == OP_LOAD_INT64) {
			...
			const auto &prev = *(std::string *) stack.back();
			stack.pop_back();
			stack.back() = *((std::string *) stack.back()) == prev;
		} else if (op == OP_S_LIKE) {
			const auto &regex = *(std::regex *) stack.back();
			stack.pop_back();
			const auto &str = *(std::string *) stack.back();
