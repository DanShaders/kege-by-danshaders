...
coro<psession> routes::_restore_session(async::pq::connection const& db, fcgx::request_t* r) {
  auto cookies_it = r->cookies.find("kege-session");
  if (cookies_it == r->cookies.end()) {
    co_return {};
  }
  auto const& session_id = cookies_it->second;
...
coro<psession> routes::require_auth(async::pq::connection const& db, fcgx::request_t* r,
                                    unsigned perms) {
...
coro<void> routes::session_store(async::pq::connection const& db, fcgx::request_t* r, psession s) {
...
coro<void> routes::session_logout(async::pq::connection const& db, psession s) {
