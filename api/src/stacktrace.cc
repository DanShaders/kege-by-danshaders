...
void init_error_cb(void*, char const* msg, int errnum) {
  loge("stacktrace init failed: {} {}", errnum, msg);
}

void stacktrace_error_cb(void*, char const*, int) {}

const exception_st empty_st{};

int const ST_INITIAL_CNT = 1;
int const ST_INITIAL_SIZE = sizeof(exception_st) + ST_INITIAL_CNT * sizeof(exception_st::st_entry);

int stacktrace_entry_cb(exception_st** pst, uintptr_t pc, char const* filename, int lineno,
                        char const* function) {
  exception_st*& st = *pst;
  if (!st) {
...
extern "C" __attribute__((nothrow)) void __cxa_throw(void* thrown_exception, std::type_info* tinfo,
  ...
  if (!inited) [[unlikely]] {
    ...
    // stacktrace of it.
    ((decltype(real__cxa_throw)) dlsym(RTLD_NEXT, "__cxa_throw"))(thrown_exception, tinfo, dest);
  }

  exception_st* st = nullptr;
  backtrace_full(state, 1, (backtrace_full_callback) stacktrace_entry_cb, stacktrace_error_cb, &st);
...
exception_st const* stacktrace::get_stacktrace(std::exception_ptr const& ptr) {
  auto st = (*(__cxxabiv1::__cxa_stacktraced_exception const**) (&ptr) - 1)->stacktrace;
  return st;
}

void stacktrace::async_update_stacktrace(std::exception_ptr const& ptr) {
...
exception_st const* stacktrace::get_stacktrace(std::exception_ptr const&) {
  return nullptr;
}

void stacktrace::async_update_stacktrace(std::exception_ptr const&) {}
#endif
