...
static char const* HEXL = "0123456789abcdef";
static int const BASE64_IDX[256] = {
...
std::string utils::url_decode(std::string_view const& str) {
  std::string ret;
...
std::string utils::url_encode(std::string_view const& str) {
  std::string ret;
  for (char c : str) {
...
parameters_map utils::parse_query_string(std::string_view const& qs) {
  parameters_map data;
...
parameters_map utils::parse_cookies(char const* s) {
  if (!s) {
    return {};
  }
  parameters_map data;
...
std::string utils::b16_encode(std::string_view const& s) {
...
std::string utils::b16_decode(std::string_view const& s) {
  std::string res(s.size() / 2, 0);
...
std::string utils::b64_decode(std::string_view const& s) {
  auto p = (unsigned char const*) s.data();
  size_t len = s.size();
...
std::string utils::b64_encode(std::string_view const& s) {
  size_t len = s.size();
  auto data = (unsigned char const*) s.data();
...
std::string utils::replace_all(std::string_view const& s, std::string_view const& from,
