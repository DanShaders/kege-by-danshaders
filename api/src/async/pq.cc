...
connection_pool::connection_pool(creation_info const& info) {
...
/* ==== async::pq::detail ==== */
coro<result> pq::detail::exec(connection_storage& c, char const* command, int size,
                              char const* values[], int lengths[], int formats[]) {
...
#define SIMPLE_PQ_DECODER(T, OID)                                                                  \
  ...
  bool pq::detail::pq_binary_converter<T>::is_valid(int oid) {                                     \
    return oid == OID;                                                                             \
                                                                                                   ...
                                                                                                   \
  template <>                                                                                      \
  T pq::detail::pq_binary_converter<T>::get([[maybe_unused]] char* data, [[maybe_unused]] int len, \
...
#define DELEGATE_PQ_CONVERSION(FROM, TO)                                                          \
  ...
  FROM pq::detail::pq_binary_converter<FROM>::get(char* data, int len, int oid) {                 \
    return (FROM){pq_binary_converter<TO>::get(data, len, oid)};                                  \
                                                                                                  ...
                                                                                                  \
  template <>                                                                                     \
  std::optional<std::string_view> pq::detail::pq_binary_converter<FROM>::set(const FROM& obj,     \
                                                                             std::string& buff) { \
...
SIMPLE_PQ_ENCODER(double) {
  uint64_t res;
  memcpy(&res, reinterpret_cast<char const*>(&obj), 8);
...
auto const PQ_EPOCH = std::chrono::sys_days{std::chrono::January / 1 / 2000};
