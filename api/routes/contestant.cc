...
namespace {
const char TOKEN_SIGN_KEY[] = {-123, 7,   78, -65, -96, -105, 117, 121,
	...
	void count_participation_status(async::pq::timestamp current_time) {
		if (current_time < start_time) {
			...
			if (is_virtual) {
				...
				} else if (*virtual_start_time <= current_time &&
						   current_time < *virtual_end_time) {
...
coro<void> handle_get_available_kims(fcgx::request_t *r) {
	...
	for (const auto &kim : co_await get_available_kims(db, session->user_id, current_time)) {
...
coro<void> handle_get_tasks(fcgx::request_t *r) {
	...
	auto kim = std::ranges::find_if(kims, [&](const auto &x) { return x.id == id; });
	...
	// Attachments
	for (auto [task_id, filename, hash, mime_type] :
		 co_await db.exec(GET_KIM_ATTACHMENTS_REQUEST)) {
...
coro<void> handle_answer(fcgx::request_t *r) {
	...
	if (kim_version != token.data.token_version) {
		auto kims = co_await get_available_kims(db, session->user_id, current_time);
		auto kim =
			std::ranges::find_if(kims, [&](const auto &x) { return x.id == token.data.kim_id; });
