...
std::string utils::hmac_sign(const std::string_view &value, const std::string_view &key) {
	unsigned char result[EVP_MAX_MD_SIZE];
	unsigned int result_len;
	HMAC(EVP_sha384(), (const unsigned char *) key.data(), (int) key.size(),
		 (const unsigned char *) value.data(), (int) value.size(), result, &result_len);
...
std::string utils::sign_url(
	...
	const std::vector<std::pair<std::string_view, std::string_view>> &params, bool full) {
...
bool utils::is_signed(fcgx::request_t *r, bool full) noexcept {
	...
	if (auto it = r->params.find("svalue"); it == r->params.end()) {
		...
		const auto &to_check = it->second, &s = r->raw_params, &key = get_sign_key(false);
