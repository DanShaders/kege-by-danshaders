...
CURLcode curl::set_url(std::string const& url) {
...
std::vector<std::string> const& curl_result::raw_headers() const {
  return storage->raw_headers;
}

std::string const& curl_result::body() const {
  return storage->body;
}

std::string const& curl_result::mime_type() const {
